...
describe 'Discounts' do
  subject { Discount.new items: order.items }
  ...
  context 'discounts' do
    ...
    it 'expects that a discount has a discount value of 0 when no discounts are defined' do
      expect(subject.calculate_discount_value).to eq(0)
    end
    ...
    context 'Percentage discount' do
      ...
      it 'expects that a discount can have a percentage of 25 defined' do
        discount2 = Discount.new(items: order.items, percent: 25)
      ...
      it 'expects that a discount can have a percentage threshold of $30 defined' do
        discount2 = Discount.new(items: order.items, percent_threshold: 30)
        expect(discount2.percent_threshold).to eq(30)
      end

      context 'Example' do
        it 'expects that an order can have a percent discount value of $10' do
          discount2 = Discount.new(items: order.items, percent: 25, percent_threshold: 30)
          broadcaster_1 = Broadcaster.new(1, 'Viacom')
          broadcaster_2 = Broadcaster.new(2, 'Disney')
          ...
          expect(discount2.calculate_discount_value).to eq(10.0)
        end
      end
    end
    ...
    context 'Delivery Discount' do
      ...
      it 'expects that a discount can have a multiple express delivery discount of $5 defined' do
        discount3 = Discount.new(items: order.items, express_delivery_discount: 5)
      ...
      it 'expects that a discount can have a multiple express delivery order quantity threshold 2 defined' do
        discount3 = Discount.new(items: order.items, express_delivery_threshold: 1)
      ...
      context 'Example' do
        it 'expects that an order can have a multi delivery discount value of $14' do
          discount3 = Discount.new(items: order.items, express_delivery_discount: 7, express_delivery_threshold: 1)
          broadcaster_1 = Broadcaster.new(1, 'Viacom')
          broadcaster_2 = Broadcaster.new(2, 'Disney')
          broadcaster_3 = Broadcaster.new(3, 'Discovery')
          ...
          expect(discount3.calculate_discount_value).to eq(14.0)
        end
      end
    end
    ...
    context 'Percent and Delivery Discount' do
      context 'Example' do
        it 'expects that an order can have percentage and multi delivery discount value of $26.5  ' do
          discount4 = Discount.new(items: order.items, percent: 25, percent_threshold: 30, express_delivery_discount: 7, express_delivery_threshold: 1)
          broadcaster_1 = Broadcaster.new(1, 'Viacom')
          broadcaster_2 = Broadcaster.new(2, 'Disney')
          broadcaster_3 = Broadcaster.new(3, 'Discovery')
          ...
          expect(discount4.calculate_discount_value).to eq(26.5)
        end
      end
    end
  end
